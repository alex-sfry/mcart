...
class AgentsList extends CBitrixComponent implements Controllerable, Errorable
{
    ...
    {   
        // $table_name = \Bitrix\Highloadblock\HighloadBlockTable::getTableName();
    ...
    private function getAgents(string $entity, array $arTypeAgents): array
    {
        ...
        $nav = new \Bitrix\Main\UI\PageNavigation(/* "nav-agents" */'bootstrap');
        ...
        while ($arAgent = $rsAgents->fetch()) {
            foreach ($arTypeAgents as $type) {
                if ($type['ID'] == $arAgent["UF_TYPE"]) {
                    $arAgent['UF_TYPE_VALUE'] = $type['VALUE'];
                    break;
                }
            }
    ...
    {   
        ...
        $userOption = CUserOptions::GetOption('mcart_agent', 'options_agents_star', false, $user_id);

        if (!$userOption) {
            $value[] = $agentID;
            CUserOptions::SetOption('mcart_agent', 'options_agents_star', $value, false, $user_id);
        }

        if ($userOption) {
            !is_array($userOption) && array($userOption);
